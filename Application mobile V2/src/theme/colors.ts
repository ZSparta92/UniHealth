...
export const palette = lightColors; // Static palette export (use useTheme() for dynamic colors)

// FAIL-FAST: Ensure palette does NOT have a .colors property (would cause collision)
if (typeof palette === 'object' && palette !== null && 'colors' in palette) {
  throw new Error('[THEME ERROR] palette must not have a .colors property. This would cause collision with useTheme().colors');
}
